using CoreAnimation;
using DllImport = TesseractOcrMaui.IOS.LeptonicaApi;
...
internal static class LeptonicaApi
{
    ...
    public static IntPtr PixRotate(HandleRef handle, float angleInRadians, RotationMethod method, RotationFill fillColor, int value1, int value2)
        => DllImport.PixRotate(handle, angleInRadians, (int)method, (int)fillColor, value1, value2);
    ...
    public static int PixWrite(string filename, HandleRef handle, ImageFormat actualFormat)
        => DllImport.PixWrite(filename, handle, (int)actualFormat);
    public static IntPtr SelCreateBrick(int selSize1, int selSize2, int v1, int v2, SelType type)
        => DllImport.SelCreateBrick(selSize1, selSize2, v1, v2, (int)type);
